...
/**
 * PantryChef Application Core Logic
 * 100% Client-Side Kitchen Copilot Engine
 */
...
(function () {
  ...
  let state = {
    pantry: ['garlic', 'rice', 'chicken-thighs', 'butter', 'olive-oil', 'soy-sauce', 'eggs', 'spinach', 'pasta', 'canned-tomatoes', 'parmesan', 'black-beans', 'tortillas', 'cheese', 'salmon', 'green-beans'],
    moodFilters: [],
    currentCategory: 'all',
    searchQuery: '',
  ...
  // --- AUDIO SYNTHESIS (Web Audio Chime for Timer) ---
  let audioCtx = null;
  ...
  const el = {
    ...
    chefScale1: document.getElementById('chefScale1'),
    chefScale2: document.getElementById('chefScale2'),
    chefScale4: document.getElementById('chefScale4')
  };
  ...
  function userHasIngredient(ingredientId) {
    ...
    // Check if user selected any parent/group
  ...
  function calculateRecipeMatch(recipe) {
    ...
    required.forEach(ing => {
      const hasDirect = userHasIngredient(ing.ingredientId);
      const isSwapActive = activeRecipeSwaps[ing.ingredientId];

      if (hasDirect || isSwapActive) {
        matchedCount++;
      } else {
        const availableSub = checkSubstitutionAvailability(ing.ingredientId);
        if (availableSub) {
          swappable.push({
            ingredient: ing,
            substitutionOption: availableSub
          });
          matchedCount += 0.9;
        } else {
          missing.push(ing);
    ...
    const score = Math.min(100, Math.round((matchedCount / required.length) * 100));

    return {
      recipe,
      score,
      totalRequired: required.length,
      matchedCount: Math.floor(matchedCount),
      missing,
      swappable,
      isPerfect: missing.length === 0 && swappable.length === 0,
      isSwappable: missing.length === 0 && swappable.length > 0
    };
  }
  ...
  function formatIngredientText(ing, multiplier) {
    ...
    let optStr = ing.optional ? ' [Optional]' : '';
  ...
  function renderRecipesGrid() {
    ...
    const readyCount = matchResults.filter(r => r.score >= 80).length;
    ...
    matchResults.forEach(({ recipe, score, missing, swappable, isPerfect, isSwappable }) => {
      ...
      let badgeHtml = '';
      if (score === 100) {
        badgeHtml = '<span class="match-badge perfect">✓ 100% Ready</span>';
      } else if (isSwappable || swappable.length > 0) {
        badgeHtml = '<span class="match-badge swappable">★ ' + score + '% Ready • ' + swappable.length + ' Swap' + (swappable.length > 1 ? 's' : '') + '</span>';
      } else if (score >= 80) {
        badgeHtml = '<span class="match-badge high">' + score + '% Ready</span>';
      } else {
        badgeHtml = '<span class="match-badge low">' + score + '% Match</span>';
      }
      ...
      } else if (missing.length > 0 && swappable.length > 0) {
        statusHtml = '<div class="match-status-row"><span class="status-missing">Missing: ' + missing.length + ' item' + (missing.length > 1 ? 's' : '') + '</span><span class="status-swappable">' + swappable.length + ' swap available</span></div>';
      } else if (missing.length > 0) {
        const missingNames = missing.map(m => getIngredientById(m.ingredientId).label).slice(0, 2).join(', ');
        statusHtml = '<div class="match-status-row"><span class="status-missing">Missing ' + missing.length + ': ' + missingNames + '</span></div>';
  ...
  function openRecipeDetail(recipeId) {
    ...
    if (matchInfo.score === 100) {
      el.detailMatchBadge.textContent = '✓ 100% Ready';
      el.detailMatchBadge.style.background = 'var(--basil-soft)';
      el.detailMatchBadge.style.color = 'var(--success)';
    } else {
      el.detailMatchBadge.textContent = matchInfo.score + '% Ready — ' + matchInfo.missing.length + ' Missing';
      el.detailMatchBadge.style.background = 'var(--butter-soft)';
      el.detailMatchBadge.style.color = 'var(--warning)';
    }
  ...
  function renderDetailIngredients(recipe) {
    ...
    recipe.ingredients.forEach(ing => {
      ...
      li.innerHTML = '<span class="ing-qty">' + fmt.qtyStr + '</span><span class="ing-name">' + fmt.name + fmt.noteStr + fmt.optStr + '</span><span class="ing-badge ' + statusClass + '">' + badgeLabel + '</span>';
      el.detailIngredientList.appendChild(li);
    });
  }

  function renderDetailSubstitutions(recipe) {
    const missing = recipe.ingredients.filter(ing => !ing.optional && !userHasIngredient(ing.ingredientId));
  ...
  // --- CHEF MODE (Distraction-Free Cooking) ---
